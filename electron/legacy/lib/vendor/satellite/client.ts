...
export class CompanionSatelliteClient
    extends EventEmitter<CompanionSatelliteClientEvents>
    implements CompanionClient
{
    ...
    private _connectionDetails: SomeConnectionDetails = {
        mode: 'tcp',
        host: '',
    ...
    private sendPing(): void {
        if (this._connected && this.socket) {
            if (
                this._pingUnackedCount > PING_UNACKED_LIMIT &&
                this._lastReceivedAt <= Date.now() - PING_IDLE_TIMEOUT
            ) {
    ...
    public addDevice(
        ...
        if (this._connected && this.socket) {
            ...
            const transferVariables = Buffer.from(
