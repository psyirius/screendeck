import type { SurfaceGraphicsContext } from '../graphics/lib'
import type {
    DeviceDrawImageFn,
    SurfaceContext,
    SurfaceId,
...
import type { ClientCapabilities, CompanionClient, DeviceRegisterProps } from '../client/types'
import { DrawingState } from '../graphics/drawingState'
import { calculateGridSize, transformButtonImage } from '../device/lib'
import {
    SatelliteConfigSize,
...
export class SurfaceProxy {
    ...
    constructor(
        ...
        if (!bitmapSize) {
            bitmapSize = Object.values(registerProps.surfaceManifest.stylePresets).find(
                (s) => !!s.bitmap
            )?.bitmap
        }
    ...
    async draw(data: SurfaceProxyDrawProps): Promise<void> {
        ...
        this.#drawQueue.queueJob(queueId, async (_key, signal) => {
            ...
            if (data.controlId !== undefined) {
                ...
                if (!controlInfo)
                    throw new Error(`Received draw for unknown controlId: ${controlId}`)

                // Interpolate a keyIndex, for compatibility
                keyIndex =
                    controlInfo.row * this.#registerProps.gridSize.columns + controlInfo.column
                row = controlInfo.row
                ...
                const controlInfo = Object.entries(
                    this.#registerProps.surfaceManifest.controls
                ).find(([_id, control]) => {
    ...
    onLockedStatus(locked: boolean, characterCount: number): void {
        ...
        if (!wasLocked) {
            ...
            this.#drawQueue.abortQueued('locked-pending-draw', async () =>
                this.#surface.blankDevice()
            )
        }
    ...
    drawPincodePage(): void {
        ...
        for (const [id, xy] of previousDraw) {
            ...
            this.#drawPincodeButton(
                xy,
                (width, height) => Buffer.alloc(width * height * 4, 0),
                '#000000',
                ''
            )
        }
        ...
        if (this.#context.pincodeMap?.type === 'multiple-page') {
            ...
            this.#drawPincodeButton(
                xy,
                (width, height) =>
                    Buffer.from(this.#graphics.locking.generatePincodeChar(width, height, '+')),
                '#ffffff',
                '+'
            )
        }
    ...
    #drawPincodeStatus() {
        ...
        this.#drawPincodeButton(
            ...
            (width, height) =>
                Buffer.from(
                    this.#graphics.locking.generatePincodeValue(
                        width,
                        height,
    ...
    #drawPincodeNumber(key: keyof SurfacePincodeMapPageEntry) {
        ...
        this.#drawPincodeButton(
            xy,
            (width, height) =>
                Buffer.from(this.#graphics.locking.generatePincodeChar(width, height, key)),
            '#ffffff',
            `${key}`
        )
    }
    ...
    #drawPincodeButton(
        ...
        const image: DeviceDrawImageFn | undefined = async (
            targetWidth,
            targetHeight,
            targetPixelFormat
        ) =>
...
export class SurfaceProxyContext implements SurfaceContext {
    ...
    keyDownById(controlId: string): void {
        ...
        if (!control) {
            this.#logger.warn(
                `Surface ${this.#surfaceId} control ${controlId} not found in keyDownById`
            )
            return
        }
    ...
    keyUpById(controlId: string): void {
        ...
        if (!control) {
            this.#logger.warn(
                `Surface ${this.#surfaceId} control ${controlId} not found in keyUpById`
            )
            return
        }
    ...
    keyDownUpById(controlId: string): void {
        ...
        if (!control) {
            this.#logger.warn(
                `Surface ${this.#surfaceId} control ${controlId} not found in keyDownUpById`
            )
            return
        }
    ...
    rotateLeftById(controlId: string): void {
        ...
        if (!control) {
            this.#logger.warn(
                `Surface ${this.#surfaceId} control ${controlId} not found in rotateLeftById`
            )
            return
        }
    ...
    rotateRightById(controlId: string): void {
        ...
        if (!control) {
            this.#logger.warn(
                `Surface ${this.#surfaceId} control ${controlId} not found in rotateRightById`
            )
            return
        }
    ...
    #pincodeXYPress(x: number, y: number): void {
        ...
        const pageInfo =
            pincodeMap.type === 'single-page' ? pincodeMap : pincodeMap.pages[this.#lockButtonPage]
        if (!pageInfo) return
